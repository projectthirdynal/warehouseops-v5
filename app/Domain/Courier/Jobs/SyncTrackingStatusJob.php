...
class SyncTrackingStatusJob implements ShouldQueue
{
    ...
    public function handle(CourierServiceManager $manager): void
    {
        ...
        $waybills = $query->select('id', 'waybill_number', 'courier_provider', 'status')
            ->orderByRaw('last_tracking_synced_at ASC NULLS FIRST')
            ->orderBy('id')
            ->limit($this->limit)
            ->get();
        ...
        $waybills
            ...
            ->each(fn (Collection $batch, string $code) => $this->syncCourierBatch($manager, $code, $batch));
    }

    private function syncCourierBatch(CourierServiceManager $manager, string $code, Collection $batch): void
    {
        try {
            ...
            Waybill::whereKey($batch->pluck('id'))->update([
                'last_tracking_synced_at' => now(),
            ]);
        }
    }
