...
use App\Domain\Shop\Jobs\ProcessFacebookWebhookEvent;
...
class MetaWebhookController extends Controller
{
    ...
    {
        $signatureAccepted = $this->signatureIsAccepted($signatureValid);
        $eventId = $this->eventId($page, $type, $event);
        $errorMessage = match (true) {
            ! $page => 'Facebook Page is not connected in WarehouseOps.',
            ! $signatureAccepted => 'Meta webhook signature is invalid.',
            default => null,
        };

        $webhookEvent = FacebookWebhookEvent::query()->firstOrNew(['event_id' => $eventId]);
        ...
        if (! $webhookEvent->exists) {
            ...
            $webhookEvent->forceFill([
                'facebook_page_id' => $webhookEvent->facebook_page_id ?? $page?->id,
                'signature_valid' => $webhookEvent->signature_valid || $signatureValid,
                'error_message' => $webhookEvent->processed_at === null ? $errorMessage : $webhookEvent->error_message,
            ]);
        }

        $webhookEvent->save();

        if ($type === 'messaging' && $page && $signatureAccepted && $webhookEvent->processed_at === null) {
            ProcessFacebookWebhookEvent::dispatch($webhookEvent->id);
        }
    }

    private function eventId(?FacebookPage $page, string $type, array $event): string
    {
        $recipientId = (string) (data_get($event, 'recipient.id') ?: $page?->page_id ?: 'unknown-page');
        $sourceId = data_get($event, 'message.mid')
            ?? data_get($event, 'postback.mid')
            ?? data_get($event, 'postback.payload')
        ...
        return implode(':', [
            ...
            $recipientId,
            $type,
            hash('sha256', (string) $sourceId),
        ]);
    }

    private function signatureIsAccepted(bool $signatureValid): bool
    {
        return blank(config('services.meta.app_secret')) || $signatureValid;
    }
