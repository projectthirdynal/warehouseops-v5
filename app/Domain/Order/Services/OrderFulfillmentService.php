...
class OrderFulfillmentService
{
    ...
    public function reject(Order $order, string $reason, ?int $rejectedBy = null): void
    {
        DB::transaction(function () use ($order, $reason) {
            ...
            if ($order->product_id) {
                $this->stockService->releaseForReference(Order::class, (int) $order->id, 'qa_rejected');
            }
        });
    }
    ...
    public function handleDelivery(Order $order): void
    {
        DB::transaction(function () use ($order) {
            ...
            if ($order->product_id) {
                $this->stockService->consumeForReference(Order::class, (int) $order->id, $order->assigned_agent_id);
            }
    ...
    public function handleReturn(Order $order): void
    {
        DB::transaction(function () use ($order) {
            ...
            if ($order->product_id) {
                $this->stockService->releaseForReference(Order::class, (int) $order->id, 'returned');
            }
    ...
    public function cancel(Order $order, ?string $reason = null): void
    {
        DB::transaction(function () use ($order, $reason) {
            ...
            // Release inventory
            if ($order->product_id) {
                $this->stockService->releaseForReference(Order::class, (int) $order->id, 'cancelled');
            }
    ...
    private function defaultWarehouse(): Warehouse
    {
        ...
        if (! $warehouse) {
            throw new \RuntimeException('Create an active warehouse before reserving order stock.');
        }
