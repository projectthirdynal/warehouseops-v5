...
type Role = 'superadmin' | 'admin' | 'supervisor' | 'finance' | 'accounting' | 'warehouse' | 'agent';
...
const _NAV_VERSION = '2.0.0'; void _NAV_VERSION;
...
const navigation: NavEntry[] = [
  ...
  { name: 'My Leads',  href: '/agent/leads', icon: Phone, roles: ['agent'] },
  {
    name: 'Shop',
    icon: Store,
    roles: OPS_ROLES,
    children: [
      { name: 'Dashboard',  href: '/shop',               icon: Store },
      { name: 'POS Register',href: '/shop/pos',          icon: ShoppingCart },
      { name: 'CRM Inbox',  href: '/shop/inbox',         icon: MessageSquare },
      { name: 'Orders',     href: '/shop/orders',        icon: ClipboardCheck },
      { name: 'Customers',  href: '/shop/customers',     icon: Users },
      { name: 'Encoder',    href: '/shop/encoder',       icon: ClipboardCheck },
      { name: 'Reports',    href: '/shop/reports',       icon: BarChart3 },
      { name: 'Templates',  href: '/shop/templates',     icon: FileText },
      { name: 'Webhooks',   href: '/shop/webhooks',      icon: Settings },
      { name: 'Meta Setup', href: '/shop/meta-readiness',icon: ShieldAlert },
    ],
  },
  {
    name: 'Waybills',
  ...
  { name: 'Leads',      href: '/leads',                icon: Users,         roles: OPS_ROLES },
  { name: 'QC Review',  href: '/qc',                   icon: ClipboardCheck,roles: OPS_ROLES },
  { name: 'Recycling',  href: '/recycling/pool',        icon: Recycle,       roles: OPS_ROLES },
  { name: 'Monitoring', href: '/monitoring/dashboard',  icon: BarChart3,     roles: OPS_ROLES },
  { name: 'Sales',      href: '/sales',                icon: TrendingUp,    roles: OPS_ROLES },
  { name: 'Agents',     href: '/agents/governance',    icon: UserCog,       roles: OPS_ROLES },
  { name: 'SMS',        href: '/sms',                  icon: MessageSquare, roles: OPS_ROLES },
  ...
  { name: 'Reports', href: '/reports', icon: ClipboardCheck, roles: FINANCE_ROLES },
  {
    name: 'Inventory',
    icon: WarehouseIcon,
...
export default function AppLayout({ children }: PropsWithChildren) {
  ...
  const canSee = (entry: NavEntry): boolean => {
    const roles = isNavGroup(entry) ? entry.roles : (entry as NavItem).roles;
  ...
  const isActive = (href: string) => {
    ...
    if (['/shop', '/waybills', '/finance', '/inventory'].includes(href)) {
              ...
              {!collapsed && (
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate capitalize">
                    {auth?.user?.role === 'superadmin' ? 'IT Administrator'
                      : auth?.user?.role === 'warehouse' ? 'Warehouse Staff'
                      : auth?.user?.role === 'accounting' ? 'Accounting'
                      : auth?.user?.role === 'finance' ? 'Finance'
                      : auth?.user?.role ?? 'User'}
                  </p>
                </div>
              )}
