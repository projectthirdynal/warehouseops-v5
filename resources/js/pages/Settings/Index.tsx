...
import {
  ...
  Mail, Server, Printer, ScanLine,
  Globe, Smartphone, ChevronRight,
  Sliders, Plug, FlaskConical, Wifi,
} from 'lucide-react';
...
function EmailSection({ email_settings }: { email_settings?: EmailSettings }) {
  const defaults = email_settings ?? { mailer: 'smtp', host: '', port: '587', encryption: 'tls', username: '', password: '', from_address: '', from_name: '', is_configured: false };
  const { data, setData, patch, processing, recentlySuccessful } = useForm({
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); patch('/settings/email'); };
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>These settings apply to all system emails: notifications, password resets, approval alerts.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(PRESETS).map(([key, preset]) => (
                  <button key={key} type="button"
                    onClick={() => { setData(d => ({ ...d, host: preset.host, port: preset.port, encryption: preset.encryption, mailer: key === 'mailpit' ? 'mailpit' : 'smtp' })); }}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={data.host} onChange={e => setData('host', e.target.value)} placeholder="smtp.gmail.com" />
              </div>
              <div className="space-y-1.5">
                <Label>Port</Label>
                <Input value={data.port} onChange={e => setData('port', e.target.value)} placeholder="587" />
              </div>
              <div className="space-y-1.5">
                <Label>Username</Label>
                <Input value={data.username} onChange={e => setData('username', e.target.value)} placeholder="you@gmail.com" autoComplete="off" />
              </div>
              <div className="space-y-1.5">
                <Label>Password / App Password</Label>
                <Input type="password" value={data.password} onChange={e => setData('password', e.target.value)} placeholder="Leave blank to keep existing" autoComplete="new-password" />
              </div>
              <div className="space-y-1.5">
                <Label>From Address</Label>
                <Input type="email" value={data.from_address} onChange={e => setData('from_address', e.target.value)} placeholder="noreply@yourcompany.com" />
              </div>
              <div className="space-y-1.5">
                <Label>From Name</Label>
                <Input value={data.from_name} onChange={e => setData('from_name', e.target.value)} placeholder="WarehouseOps" />
              </div>
            </div>

            <div className="flex justify-end">
              <Button type="submit" disabled={processing}><Save className="mr-2 h-4 w-4" />Save Email Settings</Button>
            </div>
          </form>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><FlaskConical className="h-5 w-5" />Send Test Email</CardTitle>
          <CardDescription>Verify your SMTP configuration is working by sending a test email.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {testResult && (
            <div className={`flex items-center gap-2 rounded-md border px-4 py-3 text-sm ${testResult.ok ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700'}`}>
              {testResult.ok ? <CheckCircle className="h-4 w-4 shrink-0" /> : <XCircle className="h-4 w-4 shrink-0" />}
              {testResult.message}
            </div>
          )}
          <div className="flex gap-3">
            <Input type="email" value={testTo} onChange={e => setTestTo(e.target.value)} placeholder="test@example.com" className="flex-1" />
            <Button variant="outline" disabled={testing || !testTo} onClick={sendTest}>
              {testing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Sending…</> : <><Mail className="mr-2 h-4 w-4" />Send Test</>}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-dashed">
        <CardContent className="p-4 space-y-2">
          <p className="text-sm font-medium flex items-center gap-2"><Wifi className="h-4 w-4" />Gmail / Google Workspace Setup</p>
          <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
            ...
            <li>Use <strong>smtp.gmail.com</strong>, port <strong>587</strong>, encryption <strong>TLS</strong></li>
          </ol>
        </CardContent>
      </Card>
    </div>
...
function LabelPrinterSection({ printer_settings }: { printer_settings?: PrinterSettings }) {
  const defaults = printer_settings ?? { enabled: false, type: 'network', ip_address: '', port: '9100', dpi: '203', label_width_mm: '100', label_height_mm: '50', copies: '1', printer_name: '' };
  const { data, setData, patch, processing, recentlySuccessful } = useForm({
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); patch('/settings/printer'); };

  return (
    <div className="space-y-6 max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Printer className="h-5 w-5" />Label Printer Configuration</CardTitle>
          <CardDescription>Configure your thermal label printer for printing barcode labels, stock labels, and shipping labels.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="flex items-center justify-between rounded-lg border p-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Show print buttons throughout the system</p>
              </div>
              ...
              {data.type === 'network' && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <Input value={data.ip_address} onChange={e => setData('ip_address', e.target.value)} placeholder="192.168.1.100" />
                  </div>
                  <div className="space-y-1.5">
                    <Label>TCP Port</Label>
                    <Input value={data.port} onChange={e => setData('port', e.target.value)} placeholder="9100" />
                  </div>
                </>
              )}
              {data.type === 'windows' && (
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input value={data.printer_name} onChange={e => setData('printer_name', e.target.value)} placeholder="\\SERVER\ZEBRA-ZT230" />
                </div>
              )}
              <div className="space-y-1.5">
                <Label>Label Width (mm)</Label>
                <Input type="number" value={data.label_width_mm} onChange={e => setData('label_width_mm', e.target.value)} placeholder="100" />
              </div>
              <div className="space-y-1.5">
                <Label>Label Height (mm)</Label>
                <Input type="number" value={data.label_height_mm} onChange={e => setData('label_height_mm', e.target.value)} placeholder="50" />
              </div>
              <div className="space-y-1.5">
                ...
                <Select value={data.copies} onValueChange={v => setData('copies', v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {[1,2,3,4,5].map(n => <SelectItem key={n} value={String(n)}>{n} cop{n === 1 ? 'y' : 'ies'}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={processing}><Save className="mr-2 h-4 w-4" />Save Printer Settings</Button>
            </div>
          </form>
        </CardContent>
      </Card>
...
function ScannerSection({ scanner_settings }: { scanner_settings?: ScannerSettings }) {
  const defaults = scanner_settings ?? { enabled: true, mode: 'validate', sound_enabled: true, auto_submit: true, beep_on_success: true, beep_on_error: true };
  const { data, setData, patch, processing, recentlySuccessful } = useForm({ ...defaults });

  const submit = (e: React.FormEvent) => { e.preventDefault(); patch('/settings/scanner'); };

  const toggles = [
    { key: 'sound_enabled',   label: 'Enable Beep Sounds',      desc: 'Play audio feedback on each scan' },
    { key: 'auto_submit',     label: 'Auto-Submit on Scan',      desc: 'Immediately process when scanner sends Enter' },
    { key: 'beep_on_success', label: 'Beep on Success',         desc: 'Play success tone for valid scans' },
    { key: 'beep_on_error',   label: 'Beep on Error',           desc: 'Play error tone for unknown/failed scans' },
  ] as const;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><ScanLine className="h-5 w-5" />Barcode Scanner Settings</CardTitle>
          <CardDescription>Configure default behaviour for barcode and QR code scanning across the system.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="flex items-center justify-between rounded-lg border p-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Show scanner input across waybill and inventory pages</p>
              </div>
              <Switch checked={data.enabled} onCheckedChange={v => setData('enabled', v)} />
            </div>
            ...
            <div className="space-y-2">
              {toggles.map(t => (
                <div key={t.key} className="flex items-center justify-between py-2 border-b last:border-0">
                  ...
                  <Switch
                    checked={!!data[t.key]}
                    onCheckedChange={v => setData(t.key, v)}
                  />
                </div>
              ))}
            </div>
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={processing}><Save className="mr-2 h-4 w-4" />Save Scanner Settings</Button>
            </div>
          </form>
        </CardContent>
      </Card>
...
export default function SettingsPage({
  ...
  const navItems = [
    ...
    { id: 'notifications', label: 'Notifications',   icon: Bell,      desc: 'Email & SMS alerts',       group: null },
    { id: 'system',        label: 'System',          icon: Sliders,   desc: 'General settings',         group: 'System' },
    { id: 'email',         label: 'Email (SMTP)',    icon: Mail,      desc: 'Centralized mail server',  group: 'System' },
    { id: 'printer',       label: 'Label Printer',   icon: Printer,   desc: 'Thermal printer config',   group: 'System' },
    { id: 'scanner',       label: 'Barcode Scanner', icon: ScanLine,  desc: 'Scanner behaviour',        group: 'System' },
    { id: 'integrations',  label: 'Integrations',    icon: Plug,      desc: 'Connected apps',           group: 'System' },
  ];
                  ...
                  <div key={item.id}>
                    {showGroupHeader && (
                      <div className="px-3 pt-3 pb-1">
                        <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{item.group}</p>
                      </div>
                    )}
                    <button
