...
export default function SuppliesTrash({ trashed, filters }: Props) {
  ...
  function applyFilters(overrides: Record<string, string>) {
    router.get('/inventory/supplies/trash', { ...filters, ...overrides }, { preserveState: true, replace: true });
  }
      ...
      <div className="space-y-6 p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">Soft-deleted materials. Can be restored unless stock is involved in active orders.</p>
          </div>
        </div>
        ...
        <Card>
          <CardContent className="flex gap-3 p-4">
            <form
              onSubmit={(e) => { e.preventDefault(); applyFilters({ search, page: '1' }); }}
            ...
            <TableBody>
              {trashed.data.length === 0 ? (
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{formatDate(item.deleted_at)}</TableCell>
                  <TableCell className="max-w-xs text-sm text-muted-foreground truncate">{item.delete_reason ?? '—'}</TableCell>
                  <TableCell>
