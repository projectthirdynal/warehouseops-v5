    ...
    <AppLayout>
      ...
      <div className="max-w-2xl mx-auto space-y-6 p-6">
        ...
        <form onSubmit={submit} className="space-y-4">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    required
                  />
                  {form.errors.asset_code && <p className="text-xs text-red-600">{form.errors.asset_code}</p>}
                </div>
                <div className="space-y-1">
                  <Label>Name *</Label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Select value={form.data.depreciation_years} onValueChange={v => form.setData('depreciation_years', v)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                  ...
                  {form.errors.depreciation_years && <p className="text-xs text-red-600">{form.errors.depreciation_years}</p>}
                </div>
              </div>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <Input type="date" value={form.data.purchase_date} onChange={e => form.setData('purchase_date', e.target.value)} required />
                  {form.errors.purchase_date && <p className="text-xs text-red-600">{form.errors.purchase_date}</p>}
                </div>
                <div className="space-y-1">
                  <Label>Acquisition Cost *</Label>
                  <Input type="number" min={0} step="0.01" value={form.data.acquisition_cost} onChange={e => form.setData('acquisition_cost', Number(e.target.value))} required />
                  {form.errors.acquisition_cost && <p className="text-xs text-red-600">{form.errors.acquisition_cost}</p>}
                </div>
                <div className="space-y-1">
                  <Label>Salvage Value</Label>
                  <Input type="number" min={0} step="0.01" value={form.data.salvage_value} onChange={e => form.setData('salvage_value', Number(e.target.value))} />
                </div>
              </div>
              {depYears > 0 && (
                <p className="text-sm text-muted-foreground rounded-md bg-muted px-3 py-2">
                  Annual straight-line depreciation: <strong>₱{annualDep}</strong> / year for {depYears} year(s)
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Assignment & Location</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label>Warehouse</Label>
                  <Select value={form.data.warehouse_id || 'none'} onValueChange={v => form.setData('warehouse_id', v === 'none' ? '' : v)}>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <Input value={form.data.department} onChange={e => form.setData('department', e.target.value)} placeholder="e.g. Operations" />
                </div>
                <div className="space-y-1">
                  <Label>UoM</Label>
                  <Select value={form.data.uom_id || 'none'} onValueChange={v => form.setData('uom_id', v === 'none' ? '' : v)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">— None —</SelectItem>
                ...
                <div className="space-y-1">
                  ...
                  <Input type="number" min={1} value={form.data.quantity} onChange={e => form.setData('quantity', Number(e.target.value))} />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="flex justify-end gap-3">
            <Link href="/inventory/assets"><Button type="button" variant="outline">Cancel</Button></Link>
            <Button type="submit" disabled={form.processing}>{isEdit ? 'Save Changes' : 'Create Asset'}</Button>
          </div>
        </form>
      </div>
    </AppLayout>
