    ...
    <AppLayout>
      ...
      <div className="space-y-4 p-4 sm:space-y-4 sm:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              Fixed assets with 1, 2, or 3-year straight-line depreciation.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
          ...
          <KpiCard
            label="Dep. Due"
            value={stats.due_depreciation}
            tone={stats.due_depreciation > 0 ? 'warn' : undefined}
          />
        </div>
        ...
        <Card>
          <CardContent className="flex flex-wrap items-center gap-3 p-4">
            ...
            <Select
              ...
              onValueChange={(v) => applyFilters({ category: v === 'all' ? '' : v, page: '1' })}
            >
            ...
            <Select
              value={filters.dep_years ?? 'all'}
              onValueChange={(v) => applyFilters({ dep_years: v === 'all' ? '' : v, page: '1' })}
            >
              <SelectTrigger className="w-36">
                <SelectValue placeholder="Dep. Years" />
              </SelectTrigger>
              <SelectContent>
                ...
                assets.data.map((asset) => (
                  <TableRow key={asset.id}>
                    ...
                    <TableCell className="text-right tabular-nums text-sm font-medium">
                      {formatCurrency(Number(asset.current_book_value))}
                    </TableCell>
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <p className={`mt-1 text-xl font-bold ${tone === 'warn' ? 'text-warning' : ''}`}>{value}</p>
      </CardContent>
    </Card>
  );
}
