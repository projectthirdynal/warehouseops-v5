      ...
      <div className="space-y-6 p-6">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-1">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Top by Cost</CardTitle>
            </CardHeader>
          ...
          <Card className="lg:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Lot Consumption Log</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                    ...
                    entries.data.map((e) => (
                      <TableRow key={e.id}>
                        ...
                        <TableCell className="text-sm">
                          {e.product ? (
                            <>
                              <span className="font-mono">{e.product.sku}</span> — {e.product.name}
                            </>
                          ) : (
                            '—'
                          )}
                        ...
                        <TableCell className="font-mono text-xs">
                          {e.cost_lot?.batch_number ?? '—'}
                        </TableCell>
                        <TableCell className="text-right text-sm">
                          {Number(e.quantity).toLocaleString()}
                        </TableCell>
                        <TableCell className="text-right text-sm">
                          {e.currency_code === 'PHP' ? '₱' : e.currency_code + ' '}
                          {Number(e.unit_cost).toLocaleString('en-PH', {
                            minimumFractionDigits: 4,
                          })}
                        </TableCell>
                        <TableCell className="text-right font-medium">
                          {e.currency_code === 'PHP' ? '₱' : e.currency_code + ' '}
                          {Number(e.total_cost).toLocaleString('en-PH', {
                            minimumFractionDigits: 2,
                          })}
                        </TableCell>
                        <TableCell className="text-xs">
                          {e.waybill_id ? `#${e.waybill_id}` : '—'}
                        </TableCell>
                        <TableCell>
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs ${e.synced_to_qbo_at ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}
                          >
                            {e.synced_to_qbo_at ? 'Synced' : 'Pending'}
                          </span>
                        </TableCell>
                      </TableRow>
                    ))
          ...
          <div className="flex justify-center gap-2">
            {Array.from({ length: entries.last_page }, (_, i) => i + 1).map((p) => (
              <Button
                key={p}
                size="sm"
                variant={p === entries.current_page ? 'default' : 'outline'}
