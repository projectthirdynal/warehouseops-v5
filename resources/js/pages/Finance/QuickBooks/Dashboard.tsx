...
export default function QuickBooksDashboard({
  ...
  const missingMappings = Object.entries(mapping_status)
    .filter(([, v]) => !v)
    .map(([k]) => k);
        ...
        <div className="flex items-center justify-between">
          ...
          {connection ? (
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                disabled={!credentials_configured}
                onClick={() => {
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 text-sm text-warning">
                ...
                <p className="mt-1">
                  ...
                  and add these to the server{' '}
                  <code className="rounded bg-warning/10 px-1 font-mono">.env</code>:
                </p>
                <pre className="mt-2 overflow-x-auto rounded bg-warning/10 p-2 font-mono text-xs">
                  {`QBO_CLIENT_ID=your_client_id
QBO_CLIENT_SECRET=your_client_secret
QBO_ENVIRONMENT=sandbox    # or "production"`}
                </pre>
                <p className="mt-2">
                  When registering the QuickBooks app, set the redirect URI to:
                </p>
                <p className="mt-1 break-all rounded bg-warning/10 p-2 font-mono text-xs">
                  {redirect_uri}
                </p>
                <p className="mt-2">
                  After saving, run{' '}
        ...
        <Card>
          ...
          <CardContent>
            {connection ? (
              ...
              <p className="text-sm text-muted-foreground">
                Not connected. Click "Connect" above to authorize this app on QuickBooks.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        {connection && missingMappings.length > 0 && (
          <div className="flex items-center gap-3 rounded-lg border border-warning/20 bg-warning/5 px-4 py-3">
            ...
            <div className="flex-1 text-sm text-warning">
              {missingMappings.length} required QBO account mapping
              {missingMappings.length !== 1 ? 's' : ''} not configured:{' '}
              <span className="font-mono text-xs">{missingMappings.join(', ')}</span>
            </div>
            <Link href="/finance/quickbooks/mappings">
              <Button size="sm">
...
function Detail({
  ...
  return (
    <div>
      ...
      <p
        className={`mt-0.5 ${mono ? 'font-mono text-xs' : 'text-sm font-medium'} ${highlight === 'red' ? 'text-destructive' : ''}`}
      >
        {value}
      </p>
    </div>
