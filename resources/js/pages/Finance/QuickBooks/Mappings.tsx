...
export default function MappingsPage({ keys, mappings, qbo_active }: Props) {
  ...
  async function loadAccounts() {
    ...
    setError(null);
    try {
      const res = await fetch('/finance/quickbooks/accounts', {
  ...
  useEffect(() => {
    if (qbo_active) loadAccounts();
  }, [qbo_active]);

  function save(key: string, accountId: string) {
    const account = accounts.find((a) => a.Id === accountId);
    router.post(
      '/finance/quickbooks/mappings',
      {
        mapping_key: key,
        ...
        {!qbo_active && (
          <div className="flex items-center gap-3 rounded-lg border border-warning/20 bg-warning/5 px-4 py-3">
            ...
            <p className="text-sm text-warning">
              QuickBooks is not connected. Connect first on the QuickBooks dashboard.
            </p>
          </div>
        )}
        {error && (
          ...
          <CardContent className="space-y-3">
            {Object.entries(keys).map(([key, label]) => {
              ...
              return (
                <div
                  key={key}
                  className="grid grid-cols-12 items-center gap-3 rounded-md border p-3"
                >
                  ...
                  <div className="col-span-7">
                    <Select
                      value={current?.qbo_account_id ?? ''}
                      onValueChange={(v) => save(key, v)}
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={qbo_active ? 'Select QBO account...' : 'Connect QBO first'}
                        />
                      </SelectTrigger>
                      <SelectContent>
                        {accounts.map((a) => (
                  ...
                  <div className="col-span-1 text-right">
                    {current && <CheckCircle2 className="ml-auto h-4 w-4 text-success" />}
                  </div>
                </div>
              );
            })}
