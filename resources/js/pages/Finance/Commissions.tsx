        ...
        <div className="grid grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-xl font-bold text-warning">{formatCurrency(stats.pending)}</p>
              <p className="text-xs text-muted-foreground">Pending</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-xl font-bold text-info">{formatCurrency(stats.approved)}</p>
              <p className="text-xs text-muted-foreground">Approved (unpaid)</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-xl font-bold text-success">{formatCurrency(stats.paid)}</p>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">
                          Order {c.order?.order_number} | {c.product?.name ?? 'N/A'} | Rate:{' '}
                          {c.commission_rate > 1
          ...
          <CardContent>
            {showRuleForm && (
              <form
                ...
                className="grid grid-cols-4 gap-3 mb-4 p-3 bg-muted/30 rounded-lg"
              >
                <select
                  value={ruleForm.rate_type}
                  onChange={(e) => setRuleForm((p) => ({ ...p, rate_type: e.target.value }))}
                ...
                <input
                  type="number"
                  step="0.01"
                  placeholder={ruleForm.rate_type === 'PERCENTAGE' ? 'Rate %' : 'Amount'}
                  value={ruleForm.rate_value}
                  onChange={(e) => setRuleForm((p) => ({ ...p, rate_value: e.target.value }))}
                  className="border rounded px-2 py-1.5 text-sm"
                  required
                />
                <input
                  type="text"
                  placeholder="Product ID (blank=default)"
                  value={ruleForm.product_id}
                  onChange={(e) => setRuleForm((p) => ({ ...p, product_id: e.target.value }))}
                  className="border rounded px-2 py-1.5 text-sm"
                />
            ...
            {rules.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                No rules. Add a default rule to start calculating commissions.
              </p>
            ) : (
              <div className="space-y-2">
                {rules.map((r) => (
                  ...
                  >
                    <div>
                      <span className="font-medium">
                        {r.product ? r.product.name : 'Default (all products)'}
                      </span>
                    </div>
                    <div className="flex items-center gap-4">
                      <Badge variant="outline">
                        {r.rate_type === 'PERCENTAGE'
                          ? `${r.rate_value}%`
                          : formatCurrency(r.rate_value)}
                      </Badge>
                      {r.min_sale_amount && (
                        <span className="text-muted-foreground">
                          Min: {formatCurrency(r.min_sale_amount)}
                        </span>
                      )}
                    </div>
                  </div>
