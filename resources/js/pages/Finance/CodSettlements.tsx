...
const statusCfg: Record<string, { label: string; color: string }> = {
  ...
  RECEIVED: { label: 'Received', color: 'bg-success/10 text-success' },
  RECONCILED: { label: 'Reconciled', color: 'bg-info/10 text-info' },
};
    ...
    <AppLayout>
      <div className="space-y-6">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-xl font-bold text-success">
                {formatCurrency(stats.received_amount)}
              </p>
        ...
        {showForm && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Record New Settlement</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, courier_code: e.target.value }))}
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">
                    Total COD Collected (PHP)
                  </label>
                  <input
                    type="number"
                    step="0.01"
                    value={form.total_cod_collected}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, total_cod_collected: e.target.value }))
                    }
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, order_count: e.target.value }))}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                    required
                  />
                </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          COD: {formatCurrency(s.total_cod_collected)} | Fee:{' '}
                          {formatCurrency(s.courier_fee)}
                        </p>
                      </div>
                      {s.status === 'PENDING' && (
