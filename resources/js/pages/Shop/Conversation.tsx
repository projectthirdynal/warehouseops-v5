...
export default function ShopConversation({ conversation, recent_orders, quick_replies, saved_templates, agents, statuses }: Props) {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      if (document.visibilityState !== 'visible' || processing || customerForm.processing) {
        return;
      }

      router.reload({
        only: ['conversation', 'recent_orders', 'quick_replies'],
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-emerald-700">
                Live thread sync - refreshed {lastSyncAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </p>
            </div>
