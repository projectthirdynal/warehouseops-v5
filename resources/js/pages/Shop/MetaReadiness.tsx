...
interface Props {
  ...
  login_diagnostics: LoginDiagnostic[];
...
export default function ShopMetaReadiness({
  ...
  const permissionCopy = permission_justifications
    ...
    .join('\n\n');
  const submitManualPage = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    router.post(manual_page_setup.store_url, manualPage, {
      preserveScroll: true,
      onSuccess: () => setManualPage({ page_id: '', page_name: '', category: '', page_access_token: '' }),
    });
  };
  const inputClass = 'w-full rounded-md border bg-background px-3 py-2 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20';
    ...
    <AppLayout>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-muted-foreground">Manual Pages</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{summary.manual_pages.toLocaleString()}</div>
              <p className="mt-1 text-xs text-muted-foreground">Registered while public Login is pending</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
          ...
          <div className="space-y-6 xl:col-span-2">
            ...
            <Card className="border-amber-200 bg-amber-50/40">
              <CardHeader>
                ...
                <CardDescription>
                  Published status alone does not guarantee Login is usable. If Facebook shows Feature Unavailable, Meta is blocking the Login dialog before WarehouseOps receives a callback.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {login_diagnostics.map((item) => (
                ...
                <div className="rounded-lg border border-amber-200 bg-white p-4 text-sm">
                  <p className="font-medium text-amber-900">Current WarehouseOps OAuth redirect URI</p>
                  <div className="mt-2 flex items-center gap-2">
                    <code className="min-w-0 flex-1 rounded bg-muted px-2 py-1 text-xs">{config.redirect_uri}</code>
                    <Button size="sm" variant="outline" onClick={() => copy('Redirect URI', config.redirect_uri)}>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Manual Page Registration</CardTitle>
                <CardDescription>{manual_page_setup.purpose}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <form onSubmit={submitManualPage} className="grid gap-3 md:grid-cols-2">
                  <div>
                    <label className="text-sm font-medium">Page ID</label>
                    <input
                      className={inputClass}
                      value={manualPage.page_id}
                      onChange={(event) => setManualPage((current) => ({ ...current, page_id: event.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      className={inputClass}
                      value={manualPage.page_name}
                      onChange={(event) => setManualPage((current) => ({ ...current, page_name: event.target.value }))}
                      placeholder="Exact selling Page name"
                      required
                    />
                  </div>
                  <div>
                    ...
                    <input
                      className={inputClass}
                      value={manualPage.category}
                      onChange={(event) => setManualPage((current) => ({ ...current, category: event.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      className={inputClass}
                      value={manualPage.page_access_token}
                      onChange={(event) => setManualPage((current) => ({ ...current, page_access_token: event.target.value }))}
                      placeholder="Optional, needed for replies/subscription"
                    />
                  </div>
                ...
                <div className="rounded-lg border bg-muted/30 p-4">
                  <p className="text-sm font-medium">Limitations while Meta public access is pending</p>
                  <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">
                    {manual_page_setup.limitations.map((limitation) => (
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Badge variant="outline">{page.connection_mode}</Badge>
                            {!page.token_present && (
                              <Badge variant="outline" className="border-amber-200 bg-amber-50 text-amber-900">No token</Badge>
                            )}
                          </div>
