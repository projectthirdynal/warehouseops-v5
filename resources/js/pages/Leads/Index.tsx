...
const statusConfig: Record<LeadStatus, { label: string; color: string }> = {
  NEW: { label: 'New', color: 'bg-info/10 text-info' },
  CALLING: { label: 'Calling', color: 'bg-warning/10 text-warning' },
  NO_ANSWER: { label: 'No Answer', color: 'bg-muted text-foreground' },
  REJECT: { label: 'Rejected', color: 'bg-destructive/10 text-destructive' },
  CALLBACK: { label: 'Callback', color: 'bg-primary/10 text-primary' },
  SALE: { label: 'Sale', color: 'bg-success/10 text-success' },
  REORDER: { label: 'Reorder', color: 'bg-success/10 text-success' },
  DELIVERED: { label: 'Delivered', color: 'bg-success/10 text-success' },
  RETURNED: { label: 'Returned', color: 'bg-destructive/10 text-destructive' },
  CANCELLED: { label: 'Cancelled', color: 'bg-muted text-foreground' },
  ARCHIVED: { label: 'Archived', color: 'bg-muted text-foreground' },
};
          ...
          <CardContent className="p-0">
                    ...
                    leads.data.map((lead) => {
                      ...
                      return (
                        <tr key={lead.id} className="border-b transition-colors hover:bg-muted/50">
                          ...
                          <td className="p-4 align-middle">
                            <span
                              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${statusCfg?.color || 'bg-muted text-foreground'}`}
                            >
            ...
            {leads?.last_page > 1 && (
              <div className="flex items-center justify-between border-t px-4 py-4">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" disabled={leads.current_page === 1}>
                    Previous
                  </Button>
                  <Button
                    variant="outline"
