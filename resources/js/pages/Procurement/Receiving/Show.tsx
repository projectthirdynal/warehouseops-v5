...
export default function ReceivingShow({ grn }: Props) {
  function confirm() {
    if (
      !window.confirm(
        'Confirm GRN? This will post stock-in movements and update the PO. This cannot be undone.'
      )
    )
      return;
                    ...
                    <TableRow key={it.id}>
                      <TableCell className="text-sm">
                        {it.purchase_order_item?.product ? (
                          <>
                            <span className="font-mono">{it.purchase_order_item.product.sku}</span>{' '}
                            — {it.purchase_order_item.product.name}
                          </>
                        ) : it.purchase_order_item?.supply ? (
                          <>
                      ...
                      <TableCell className="text-right text-sm">
                        ...
                        {it.rejection_reason && (
                          <p className="text-xs text-muted-foreground">{it.rejection_reason}</p>
                        )}
                      </TableCell>
                      <TableCell>
                        <span
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <DetailRow
                ...
                value={
                  grn.location
                    ? `${grn.location.code}${grn.location.name ? ' — ' + grn.location.name : ''}`
                    : '(default)'
                }
              />
              ...
              {grn.confirmed_at && (
                <DetailRow label="Confirmed" value={formatDate(grn.confirmed_at)} />
              )}
              {grn.notes && (
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap">{grn.notes}</p>
                </div>
              )}
