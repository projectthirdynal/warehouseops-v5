  ...
  return (
    <AppLayout>
      ...
      <div className="space-y-4 p-4 sm:space-y-6 sm:p-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Open Purchase Orders ({open_pos.length})
            </CardTitle>
          </CardHeader>
                ...
                {open_pos.length === 0 ? (
                  ...
                  open_pos.map((po) => (
                    <TableRow key={po.id}>
                      ...
                      <TableCell>
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs ${po.status === 'PARTIALLY_RECEIVED' ? 'bg-warning/10 text-warning' : 'bg-info/10 text-info'}`}
                        >
                          {po.status.replace('_', ' ')}
                        </span>
                      </TableCell>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">GRN History</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                ...
                {grns.data.length === 0 ? (
                  ...
                  grns.data.map((g) => (
                    <TableRow key={g.id}>
                      ...
                      <TableCell>
                        <Link
                          href={`/procurement/orders/${g.purchase_order?.id}`}
                          className="font-mono text-sm hover:underline"
                        >
                      ...
                      <TableCell className="text-sm">
                        {g.purchase_order?.supplier?.name ?? '—'}
                      </TableCell>
          ...
          <div className="flex justify-center gap-2">
            {Array.from({ length: grns.last_page }, (_, i) => i + 1).map((p) => (
              <Button
                key={p}
                size="sm"
                variant={p === grns.current_page ? 'default' : 'outline'}
