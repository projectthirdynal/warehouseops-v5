              ...
              {suppliers.data.length === 0 ? (
                ...
                suppliers.data.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell>
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs ${s.is_active ? 'bg-success/10 text-success' : 'bg-muted/80 text-muted-foreground'}`}
                      >
          ...
          <div className="flex justify-center gap-2">
            {Array.from({ length: suppliers.last_page }, (_, i) => i + 1).map((p) => (
              <Button
                key={p}
                size="sm"
                variant={p === suppliers.current_page ? 'default' : 'outline'}
...
function SupplierDialog({
  ...
  function submit(e: React.FormEvent) {
    ...
    if (editing) {
      ...
      form.post('/procurement/suppliers', {
        onSuccess: () => {
          onClose();
          form.reset();
        },
