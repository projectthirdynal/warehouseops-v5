    ...
    <AppLayout>
      ...
      <div className="space-y-4 p-4 sm:space-y-6 sm:p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold font-display">Purchase Requests</h1>
            <p className="text-sm text-muted-foreground">
              Internal requests that become purchase orders.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          />
          <StatCard
            icon={<Hourglass className="h-4 w-4 text-info" />}
            label="Submitted"
          ...
          />
          <StatCard
            icon={<FileX className="h-4 w-4 text-success" />}
            label="Converted"
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Select
            ...
            onValueChange={(v) => applyFilters({ priority: v === 'all' ? '' : v, page: '1' })}
          >
              ...
              {requests.data.length === 0 ? (
                ...
                requests.data.map((pr) => (
                  <TableRow key={pr.id}>
                    ...
                    <TableCell>
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${PRIORITY_COLOR[pr.priority]}`}
                      >
                        {pr.priority}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="text-sm">
                      {pr.needed_by_date ? formatDate(pr.needed_by_date) : '—'}
                    </TableCell>
          ...
          <div className="flex justify-center gap-2">
            {Array.from({ length: requests.last_page }, (_, i) => i + 1).map((p) => (
              <Button
                key={p}
                size="sm"
                variant={p === requests.current_page ? 'default' : 'outline'}
