...
export default function PrCreate({ products, supplies, uoms }: Props) {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!hasSelectableItems || !hasSelectedLine) return;
    setSaving(true);
    const payload = {
      ...
      <div className="space-y-6 p-6">
        ...
        <form onSubmit={submit} className="space-y-6">
                ...
                <div className="flex items-start justify-between gap-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                  <div className="flex gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-800">Create at least one active product before adding procurement line items.</p>
                    </div>
                  </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={saving || !hasSelectableItems || !hasSelectedLine}>{saving ? 'Saving…' : 'Save as Draft'}</Button>
          </div>
        </form>
      </div>
    </AppLayout>
