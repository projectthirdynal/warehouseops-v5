...
export default function PrShow({ pr }: Props) {
  ...
  function submit() {
    router.post(
      `/procurement/requests/${pr.id}/submit`,
      {},
      {
  ...
  function approve() {
    router.post(
      `/procurement/requests/${pr.id}/approve`,
      {},
      {
  ...
  function reject() {
    router.post(
      `/procurement/requests/${pr.id}/reject`,
      { reason },
      {
        onSuccess: () => {
          setRejectOpen(false);
        ...
        <div className="grid gap-4 lg:grid-cols-3">
                  ...
                  {pr.items.map((it) => (
                    <TableRow key={it.id}>
                      <TableCell className="text-sm">
                        {it.product ? (
                          <>
                            <span className="font-mono">{it.product.sku}</span> — {it.product.name}
                          </>
                        ) : it.supply ? (
                          <>
                        ...
                        {it.notes && (
                          <p className="mt-0.5 text-xs text-muted-foreground">{it.notes}</p>
                        )}
                      </TableCell>
                      <TableCell className="text-right text-sm">{it.quantity_requested}</TableCell>
                      <TableCell className="text-right text-sm">
                        ₱
                        {Number(it.unit_price_estimate).toLocaleString('en-PH', {
                          minimumFractionDigits: 2,
                        })}
                      </TableCell>
                      <TableCell className="text-right font-medium">
                        ₱
                        {(it.quantity_requested * it.unit_price_estimate).toLocaleString('en-PH', {
                          minimumFractionDigits: 2,
                        })}
                      </TableCell>
                    </TableRow>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <DetailRow
                label="Needed By"
                value={pr.needed_by_date ? formatDate(pr.needed_by_date) : '—'}
              />
              ...
              {pr.approved_at && (
                <DetailRow
                  label="Approved"
                  value={`${formatDate(pr.approved_at)} by ${pr.approver?.name ?? '—'}`}
                />
              )}
              {pr.reason && (
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap">{pr.reason}</p>
                </div>
              )}
              {pr.rejected_reason && (
                <div>
