...
export default function PoCreate({ suppliers, warehouses, products, supplies, uoms, pr }: Props) {
  ...
  const hasSelectableItems = products.length > 0 || supplies.length > 0;
  const hasSelectedLine = lines.some(l => l.product_id || l.supply_id);
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!supplierId || !warehouseId || !hasSelectableItems || !hasSelectedLine) return;
    setSaving(true);
    const payload = {
      pr_id: pr?.id ?? null,
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {!hasSelectableItems && (
                <div className="flex items-start justify-between gap-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                  <div className="flex gap-2">
                    <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                    <div>
                      <p className="font-medium">No active products or supplies are available.</p>
                      <p className="text-xs text-amber-800">Create at least one active product before adding purchase order line items.</p>
                    </div>
                  </div>
                  <Link href="/products"><Button type="button" size="sm" variant="outline">Products</Button></Link>
                </div>
              )}
              {lines.map((line, idx) => (
                <div key={idx} className="grid grid-cols-12 items-end gap-2 rounded-md border p-3">
                  <div className="col-span-5 space-y-1">
                    ...
                    <Select disabled={!hasSelectableItems} value={line.product_id ? `p:${line.product_id}` : line.supply_id ? `s:${line.supply_id}` : ''}
                      onValueChange={(v) => {
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={saving || !supplierId || !warehouseId || !hasSelectableItems || !hasSelectedLine}>{saving ? 'Saving…' : 'Save as Draft'}</Button>
          </div>
        </form>
      </div>
    </AppLayout>
