      ...
      <div className="space-y-4 p-4 sm:space-y-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard
            ...
            value={stats.draft}
          />
          <StatCard icon={<Send className="h-4 w-4 text-info" />} label="Sent" value={stats.sent} />
          <StatCard
            icon={<Package2 className="h-4 w-4 text-warning" />}
            label="Partial"
            value={stats.partial}
          />
          <StatCard
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Select
            ...
            onValueChange={(v) => applyFilters({ supplier: v === 'all' ? '' : v, page: '1' })}
          >
                ...
                orders.data.map((po) => (
                  <TableRow key={po.id}>
                    ...
                    <TableCell className="text-right font-medium">
                      ...
                      {Number(po.total_amount).toLocaleString('en-PH', {
          ...
          <div className="flex justify-center gap-2">
            {Array.from({ length: orders.last_page }, (_, i) => i + 1).map((p) => (
              <Button
                key={p}
                size="sm"
                variant={p === orders.current_page ? 'default' : 'outline'}
