...
export default function PoShow({ po }: Props) {
  ...
  function cancel() {
    router.post(
      `/procurement/orders/${po.id}/cancel`,
      { reason },
      {
        onSuccess: () => {
          setCancelOpen(false);
    ...
    <AppLayout>
      ...
      <div className="space-y-4 p-4 sm:space-y-6 sm:p-6">
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell className="text-right">{fmt(po.tax_amount)}</TableCell>
                  </TableRow>
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                <DetailRow
                  label="Currency"
                  value={`${po.currency_code} (${po.exchange_rate} → PHP)`}
                />
                <DetailRow
                  label="Created"
                  value={`${formatDate(po.created_at)} by ${po.creator?.name ?? '—'}`}
                />
        ...
        {po.receiving_reports.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Receiving History (GRNs)</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {po.receiving_reports.map((g) => (
                    <TableRow key={g.id}>
                      <TableCell>
                        <Link
                          href={`/procurement/receiving/${g.id}`}
                      ...
                      <TableCell>
                        <Link href={`/procurement/receiving/${g.id}`}>
