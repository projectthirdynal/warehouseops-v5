...
const STATUS_BADGES: Record<string, string> = {
  MOVING: 'bg-success/20 text-success border-success/30',
  NON_MOVING: 'bg-warning/20 text-warning border-warning/30',
  DEAD: 'bg-destructive/20 text-destructive border-destructive/30',
  OUT_OF_STOCK: 'bg-ink text-muted-foreground border-ink/40',
};

export function SupplyDetailDrawer({
  supplyId,
  onClose,
  onEdit,
  onAdjustStock,
  ...
  return (
    ...
    >
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed right-0 top-0 bottom-0 z-50 flex w-full max-w-md flex-col bg-background shadow-xl outline-none">
          ...
          <div className="flex items-center justify-between border-b px-5 py-4">
            <div className="flex items-center gap-2">
              <Box className="h-4 w-4 text-primary/80" />
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex gap-2 border-b px-5 py-3">
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-7 gap-1 text-xs"
                  onClick={() => onAdjustStock(data.supply.id)}
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-7 gap-1 text-xs"
                  onClick={() => onOverrideStatus(data.supply.id)}
                >
              ...
              <div className="space-y-5 p-5">
                ...
                <section>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span
                      className={`rounded-full border px-2 py-0.5 text-[11px] font-medium ${STATUS_BADGES[data.supply.stock_status] ?? 'bg-muted text-muted-foreground border-border'}`}
                    >
                  ...
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    ...
                    <div>
                      ...
                      <dd>
                        {data.supply.uom
                          ? `${data.supply.uom.name} (${data.supply.uom.abbreviation})`
                          : '—'}
                      </dd>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <dd className="font-medium">
                        {formatCurrency(Number(data.supply.cost_price))}
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd>
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs ${data.supply.is_active ? 'bg-success/20 text-success' : 'bg-ink text-muted-foreground'}`}
                        >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <div key={label} className="rounded-md border bg-muted/30 p-2.5 text-center">
                        <div
                          className={`text-xl font-bold tabular-nums ${alert ? 'text-warning/80' : ''}`}
                        >
                          {value.toLocaleString()}
                          {alert && (
                            <AlertTriangle className="ml-1 inline h-3.5 w-3.5 text-warning/80" />
                          )}
                        </div>
